...
import pytest
from chestxraytrainer import ChestXrayTrainer
from chestxrayvalidator import ChestXRayValidator
...
def get_cifar_trainer():
    with patch.object(ChestXrayTrainer, "_save_local_model") as mock_save:
        with patch.object(ChestXrayTrainer, "_load_local_model") as mock_load:
            yield ChestXrayTrainer(train_task_name=TRAIN_TASK_NAME, epochs=1)
...
class TestCifar10Trainer:
    ...
    @patch.object(ChestXrayTrainer, "_save_local_model")
    @patch.object(ChestXrayTrainer, "_load_local_model")
    def test_execute_rounds(self, mock_save, mock_load):
        train_task_name = "train"
        trainer = ChestXrayTrainer(train_task_name=train_task_name, epochs=2)
        # just take first batch
...
class TestCifar10Validator:
    def test_execute(self):
        ...
        validator = ChestXrayValidator(validate_task_name=validate_task_name)
        # just take first batch
